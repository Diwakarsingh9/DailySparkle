 ...
 */
package com.apporio.dailysparkle.pdfviewer;
...
import com.apporio.dailysparkle.PhotoInnerActivity;
import com.apporio.dailysparkle.pdfviewer.scroll.ScrollHandle;

import static com.apporio.dailysparkle.pdfviewer.util.Constants.Pinch.MAXIMUM_ZOOM;
import static com.apporio.dailysparkle.pdfviewer.util.Constants.Pinch.MINIMUM_ZOOM;
...
class DragPinchManager implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener, ScaleGestureDetector.OnScaleGestureListener, View.OnTouchListener {
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        if (wantedZoom < MINIMUM_ZOOM) {
            if(PhotoInnerActivity.pageno==0) {
                PhotoInnerActivity.llforhead.setVisibility(View.VISIBLE);
            }
            dr = MINIMUM_ZOOM / pdfView.getZoom();
        }
//        else if(wantedZoom<2){
//            if(PhotoInnerActivity.pageno==0) {
//                PhotoInnerActivity.llforhead.setVisibility(View.VISIBLE);
//            }
...
//            Toast.makeText(PhotoInnerActivity.pdp, "max zoom", Toast.LENGTH_SHORT).show();

            dr = MAXIMUM_ZOOM / pdfView.getZoom();
        }
        else {
            if(PhotoInnerActivity.pageno==0) {
                PhotoInnerActivity.llforhead.setVisibility(View.GONE);
            }
        }
